      ...
      <section class="ai-case-screen ai-case-demo">
        ...
        <div class="demo-phone-stage reveal" style="--delay:100ms">
          <div class="demo-phone-frame">
            <iframe src="./ecommerce-demo/index.html?v=20260728g" title="AI 电商搜索交互 Demo" loading="eager"></iframe>
          </div>
        </div>
      </section>
...
function setupDemoFramePosition() {
  ...
  function createOverlay() {
    const stage = document.querySelector(".demo-phone-stage");
    if (!stage) return;

    demoCursor = document.createElement("div");
    demoCursor.className = "auto-demo-cursor";
    demoCursor.innerHTML = '<span class="auto-demo-cursor-dot"></span>';
    stage.appendChild(demoCursor);

    demoBadge = document.createElement("div");
    demoBadge.className = "auto-demo-badge";
    demoBadge.innerHTML = '<span class="auto-demo-dot"></span> 自动演示中';
    stage.appendChild(demoBadge);
  }

  function positionCursor(targetRect) {
    if (!demoCursor || !frame) return;
    const frameRect = frame.getBoundingClientRect();
    // Account for iframe internal scaling (375px width in the iframe)
    const scaleX = frameRect.width / 375;
    const scaleY = frameRect.height / 812;
    const cx = frameRect.left + targetRect.left * scaleX + (targetRect.width * scaleX) / 2;
    const cy = frameRect.top + targetRect.top * scaleY + (targetRect.height * scaleY) / 2;
    demoCursor.style.left = cx + "px";
    demoCursor.style.top = cy + "px";
  }

  function showCursor() {
    if (demoCursor) demoCursor.classList.add("visible");
  }
  function hideCursor() {
    if (demoCursor) demoCursor.classList.remove("visible");
  }
  function showBadge() {
    if (demoBadge) demoBadge.classList.add("visible");
  }
  function hideBadge() {
    if (demoBadge) demoBadge.classList.remove("visible");
  }

  function getClickables(doc) {
    const all = doc.querySelectorAll("div, button, a, span, img, p, h1, h2, h3, li, input, textarea");
    const results = [];
    for (let i = 0; i < all.length; i++) {
      ...
      if (style.cursor === "pointer" || el.tagName === "BUTTON" || el.tagName === "A" || el.getAttribute("role") === "button") {
        ...
        if (rect.width > 8 && rect.height > 8 &&
            rect.top < 812 && rect.bottom > 0 &&
            rect.left < 375 && rect.right > 0 &&
  ...
  function tapElement(el, cb) {
    positionCursor(el.getBoundingClientRect());
    showCursor();
    demoCursor.classList.add("tap");
    setTimeout(() => {
      try { el.click(); } catch (e) {}
      demoCursor.classList.remove("tap");
      demoClicked.add(el);
      if (cb) setTimeout(cb, 400);
    }, 350);
  }

  function runStep(doc) {
    if (!demoRunning) return;

    const fresh = getClickables(doc).filter(el => !demoClicked.has(el));
    if (fresh.length === 0) {
      // Flow complete or transitioning
      demoCycle++;
      if (demoCycle < MAX_CYCLES) {
        // Try to reset by clicking a back button
        const backBtns = doc.querySelectorAll('[class*="back"], [class*="Back"], [class*="close"], [class*="Close"]');
        for (let i = 0; i < backBtns.length; i++) {
          try { backBtns[i].click(); break; } catch (e) {}
        }
        demoClicked = new Set();
        demoTimeout = setTimeout(() => runStep(doc), 2500);
        return;
      }
      // Demo finished
      stopDemo();
    ...
    demoTimeout = setTimeout(() => {
      tapElement(el, () => {
        demoTimeout = setTimeout(() => runStep(doc), 600 + Math.random() * 700);
      });
    }, delay);
  }
  ...
  function startDemo() {
    ...
    try {
      ...
      if (!doc || !doc.body || !doc.querySelector("#root > *")) return;
      demoRunning = true;
      createOverlay();
      showBadge();
      demoTimeout = setTimeout(() => runStep(doc), 1800);
    } catch (e) {}
  }
  ...
  function cleanup() {
    if (demoCursor) { demoCursor.remove(); demoCursor = null; }
  }

  // Listen for user clicks in the iframe to pause auto-demo
  function onUserClick(e) {
    if (!demoRunning) return;
    const target = e.target;
    const style = frame.contentDocument?.defaultView?.getComputedStyle(target);
    if (style && (style.cursor === "pointer" || target.closest("button, a, [role='button']"))) {
      // User interacted - pause and restart cycle
      if (demoTimeout) clearTimeout(demoTimeout);
      demoClicked = new Set();
      demoCycle = 0;
      if (demoBadge) {
        demoBadge.innerHTML = '<span class="auto-demo-dot" style="background:#FF9500"></span> 已暂停（3秒后恢复）';
      }
      if (demoCursor) demoCursor.classList.add("pause");
      demoTimeout = setTimeout(() => {
        if (demoCursor) demoCursor.classList.remove("pause");
        if (demoBadge) demoBadge.innerHTML = '<span class="auto-demo-dot"></span> 自动演示中';
        try { runStep(frame.contentDocument); } catch (e) {}
      }, 3000);
    }
  }
  ...
  frame.addEventListener("load", () => {
    ...
    try {
      frame.contentDocument?.addEventListener("click", onUserClick, true);
    } catch (e) {}
  }, { once: true });
}
